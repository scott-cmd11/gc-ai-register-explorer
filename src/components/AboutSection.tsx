...
export default function AboutSection() {
  ...
  // dangerouslySetInnerHTML is used below for translation strings containing
  // <strong> tags. These are hardcoded developer-controlled constants in i18n.tsx,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          ...
          <div className="col-span-1 lg:col-span-2 space-y-4">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="h-7 w-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--accent)', color: '#fff' }}>
            ...
            <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)', lineHeight: 1.7 }}>
              ...
              <a href="https://www.linkedin.com/in/scott-hazlitt/" target="_blank" rel="noopener noreferrer"
                className="underline underline-offset-2 hover:opacity-70 transition-opacity font-medium" style={{ color: 'var(--accent-text)' }}>
                Scott Hazlitt
              </a>{t('about_bio')}
            </p>
            <div className="pt-2">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-bold tracking-wider uppercase mb-4" style={{ color: 'var(--text-tertiary)', letterSpacing: '0.08em' }}>{t('methodology')}</h3>
            <ul className="space-y-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
              <li className="flex gap-3">
                <div className="h-7 w-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
                  <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                  </svg>
                </div>
                <span>
                  <strong className="block text-sm mb-0.5" style={{ color: 'var(--text-primary)' }}>{t('data_source_label')}</strong>
                  <span className="text-xs leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>
                    {t('data_source_text')}{' '}
                    <a href={openCanadaUrl}
                      target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:opacity-70 transition-opacity" style={{ color: 'var(--accent-text)' }}>
                      open.canada.ca
                    </a>{' '}
              ...
              <li className="flex gap-3">
                ...
                <span>
                  <strong className="block text-sm mb-0.5" style={{ color: 'var(--text-primary)' }}>{t('updates_label')}</strong>
                  <span className="text-xs leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>{t('updates_text')}</span>
                </span>
              </li>
              <li className="flex gap-3">
                <div className="h-7 w-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
                ...
                <span>
                  <strong className="block text-sm mb-0.5" style={{ color: 'var(--text-primary)' }}>{t('licence_label')}</strong>
                  <a href={licenceUrl}
                    target="_blank" rel="noopener noreferrer" className="text-xs underline underline-offset-2 hover:opacity-70 transition-opacity" style={{ color: 'var(--accent-text)' }}>
                    {t('licence_link')}
                  </a>
                </span>
              </li>
